...
create policy scheme_applications_staff_read on public.scheme_applications for select to authenticated
  using (public.is_admin(auth.uid()) or exists (select 1 from public.leads l where l.id=lead_id and l.assigned_to=auth.uid()));
...
create policy application_documents_staff_read on public.application_documents for select to authenticated
  using (public.is_admin(auth.uid()) or exists (select 1 from public.scheme_applications a join public.leads l on l.id=a.lead_id where a.id=application_id and l.assigned_to=auth.uid()));
...
begin
  ...
  insert into public.scheme_applications(lead_id,consumer_number,state,discom,application_reference)
  values (lead_uuid,left(info->>'consumer_number',40),left(info->>'state',80),left(info->>'discom',120),nullif(left(info->>'application_reference',80),''))
...
create policy customer_documents_upload on storage.objects for insert to anon,authenticated with check (
  bucket_id='customer-documents' and exists (select 1 from public.scheme_applications a where a.id::text=(storage.foldername(name))[1])
);
...
create policy visit_evidence_upload on storage.objects for insert to authenticated with check (
  bucket_id='site-visit-evidence' and (storage.foldername(name))[2]=auth.uid()::text and exists (
    select 1 from public.tasks t where t.id::text=(storage.foldername(name))[1] and t.assigned_to=auth.uid() and t.task_type='site_visit')
);
